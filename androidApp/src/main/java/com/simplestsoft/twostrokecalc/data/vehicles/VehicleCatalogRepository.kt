...
@Singleton
class VehicleCatalogRepository @Inject constructor(
    @ApplicationContext private val context: Context,
    private val gson: Gson,
    private val firestoreSync: VehicleCatalogFirestoreSync,
) {
    private val mutex = Mutex()
    private var catalog: VehicleCatalogFile = loadInitialCatalog()
    ...
    fun yearsForBrandModel(brand: String, model: String): List<Int> {
        ...
        return (minYear..maxYear).filter { year ->
            entries.any { it.matchesYear(year) }
        }
    }
    ...
    private fun loadInitialCatalog(): VehicleCatalogFile =
        firestoreSync.loadCacheFile()
            ?: loadBundledCatalog()

    private fun loadBundledCatalog(): VehicleCatalogFile = runCatching {
        context.assets.open(ASSET_FILE).bufferedReader().use { reader ->
            gson.fromJson(reader, VehicleCatalogFile::class.java)
        }
    }.getOrElse { VehicleCatalogFile() }

    companion object {
        const val ASSET_FILE = "vehicle_catalog.json"
    }
}
